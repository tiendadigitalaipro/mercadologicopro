  ...
  Future<void> _activar() async {
    ...
    if (mounted && ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('✅ Licencia activada'), backgroundColor: Colors.green),
      );
    }
  }
        ...
        child: Center(
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                const Icon(Icons.storefront, color: AppColors.harvest, size: 56),
                const SizedBox(height: 12),
                const Text('Mercado Logic Pro', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                Text(
                  licencia.mensajeError != null ? 'Tu licencia no está activa' : 'Ingresa tu código para empezar',
                      ...
                      children: [
                        TextField(
                          ...
                          decoration: const InputDecoration(
                            ...
                            hintText: 'PRO-MERCADO-XXXXXXXX',
                          ),
                        ),
                        if (licencia.mensajeError != null) ...[
                          const SizedBox(height: 8),
                          Text(licencia.mensajeError!, style: const TextStyle(color: Colors.redAccent, fontSize: 12)),
                        ],
                ...
                OutlinedButton.icon(
                  ...
                  label: Text('Escríbenos al $_whatsappVentas'),
                  style: OutlinedButton.styleFrom(foregroundColor: AppColors.harvest, side: const BorderSide(color: AppColors.harvest)),
                ),
