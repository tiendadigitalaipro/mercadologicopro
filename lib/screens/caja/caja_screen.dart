...
class CajaScreen extends StatelessWidget {
  ...
  ({double total, double efectivo, double otros}) _resumen(List<Venta> ventas, DateTime openAt) {
    final delTurno = ventas.where((v) => !v.date.isBefore(openAt)).toList();
    return (
      total: delTurno.fold(0.0, (s, v) => s + v.total),
      efectivo: delTurno.where((v) => v.method.contains('Efectivo')).fold(0.0, (s, v) => s + v.total),
      otros: delTurno.where((v) => !v.method.contains('Efectivo')).fold(0.0, (s, v) => s + v.total),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value, style: TextStyle(color: destacado ? AppColors.harvest : Colors.white, fontWeight: destacado ? FontWeight.bold : FontWeight.normal, fontSize: destacado ? 15 : 13)),
        ],
      ),
    );
  }
